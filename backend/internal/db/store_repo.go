...
func (r *StoreRepo) Get() (*models.StoreInfo, error) {
	row := r.db.QueryRow(`SELECT id, name, COALESCE(description,''), COALESCE(logo_url,''), COALESCE(phone,''), COALESCE(email,''), COALESCE(address,'') FROM store_info WHERE id = 1`)
	info := &models.StoreInfo{}
	if err := row.Scan(&info.ID, &info.Name, &info.Description, &info.LogoURL, &info.Phone, &info.Email, &info.Address); err != nil {
		return nil, err
	}
	return info, nil
}
...
func (r *StoreRepo) Update(name, description, logoURL, phone, email, address string) (*models.StoreInfo, error) {
	_, err := r.db.Exec(`UPDATE store_info SET name = ?, description = ?, logo_url = ?, phone = ?, email = ?, address = ? WHERE id = 1`,
